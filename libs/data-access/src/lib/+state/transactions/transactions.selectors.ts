...
export const selectTotalBalance = createSelector(
  ...
  (transactions) => transactions.reduce((acc, t) => t.type === 'income' ? acc + t.amount : acc - t.amount, 0)
);

export const selectIncomeTotal = createSelector(
  selectAllTransactions,
  (transactions) => transactions.filter(t => t.type === 'income').reduce((acc, t) => acc + t.amount, 0)
);

export const selectExpenseTotal = createSelector(
  selectAllTransactions,
  (transactions) => transactions.filter(t => t.type === 'expense').reduce((acc, t) => acc + t.amount, 0)
);
